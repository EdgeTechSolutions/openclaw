...
- **Not in ~/.openclaw/credentials/** — tokens are stored in the skill directory for sandbox compatibility

## Veiligheid

- **Lokaal**: Alle tokens worden lokaal opgeslagen in `skills/microsoft365/tokens/ms365.tokens.default.json`.
...
4. Ga in het linkermenu naar **Authentication**.
   ...
   - Klik **Save**.
5. Maak een bestand `config.default.json` in de `skills/microsoft365/` map met de volgende inhoud (vervang `YOUR_CLIENT_ID`):
...
5. Het script pikt automatisch de login op en slaat de tokens veilig op in `skills/microsoft365/tokens/ms365.tokens.default.json`.
...
- **Token expired**: Verwijder `skills/microsoft365/tokens/ms365.tokens.default.json` en log opnieuw in.
